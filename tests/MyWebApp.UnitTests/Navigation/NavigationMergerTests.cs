...
public sealed class NavigationMergerTests
{
    ...
    [Fact]
    public void ResolvesProviderTypeUsingLoadedAssemblyWhenAssemblyNameIsLegacy()
    {
        var options = Options.Create(new SubmenuCatalogsOptions
        {
            Catalogs = new Dictionary<string, SubmenuCatalogEntry>
            {
                ["SalesReports"] = new SubmenuCatalogEntry
                {
                    ProviderType = $"{typeof(ReportMockService).FullName}, MyWebApp",
                    ParentRoute = "/reports",
                    Icon = "calendar_month"
                }
            }
        ...
        var factory = new SubmenuProviderFactory(
            new FakeServiceProvider((typeof(ReportMockService), new ReportMockService())),
            options);

        var provider = factory.GetProvider("SalesReports");

        Assert.IsType<ReportMockService>(provider);
    }
